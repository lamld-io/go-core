...
func TestVerifyEmailThenLogin(t *testing.T) {
	...
	user, tokenPair, err := svc.Login(ctx, "verify@example.com", "password123")
	require.NoError(t, err)
...
func TestForgotAndResetPassword(t *testing.T) {
	...
	_, oldTokenPair, err := svc.Login(ctx, "reset@example.com", "password123")
	require.NoError(t, err)
	...
	_, _, err = svc.Login(ctx, "reset@example.com", "password123")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "invalid email or password")

	_, _, err = svc.Login(ctx, "reset@example.com", "newpassword123")
	require.NoError(t, err)

	_, err = svc.RefreshToken(ctx, oldTokenPair.RefreshToken)
	require.Error(t, err)
}
...
func TestLoginLockoutPolicyIsRuntimeConfigurable(t *testing.T) {
	...
	_, _, err = svc.Login(ctx, "lock@example.com", "wrong-password")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "invalid email or password")
	...
	_, _, err = svc.Login(ctx, "lock@example.com", "password123")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "account is locked until")
